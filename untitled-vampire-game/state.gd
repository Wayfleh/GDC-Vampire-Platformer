...
signal transition(next: String) #tell state machine to switch to the next state based on name
...
func _ready() -> void:
	var sm = get_parent() #states will never not be a child of statemachines
